...
export default class SocketHandler {
  constructor(scene) {
    ...
    this.socket.on("updateHP", (data) => {
      ...
      this.scene.uiHandler.updateHP(data.playerId, data.newHP);
    });
    ...
    this.socket.on("gameOver", (winnerId) => {
      ...
      this.scene.scene.start("GameOver", { winnerId });
    });
  }

  startMatchmaking(scene) {
    scene.searchingText.setVisible(true);
    scene.cancelButton.setVisible(true);
    
    this.socket.emit("findOpponent");
    
    this.socket.on("opponentFound", (roomId) => {
      console.log(`Match found! Joining room: ${roomId}`);
      scene.scene.start("BuildATerror", { roomId });
    });
  }
  ...
  cancelMatchmaking() {
    ...
    if (this.scene.searchingText) {
      this.scene.searchingText.setVisible(false);
    }
    if (this.scene.cancelButton) {
      this.scene.cancelButton.setVisible(false);
    }
  }
